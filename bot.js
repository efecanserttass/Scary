...
bot.on('guildMemberAdd', member => { bot.channels.get('451091996287696910').setName(`Toplam Üye: ${member.guild.memberCount}`) let humans = member.guild.members.filter(m => !m.user.bot).size; bot.channels.get('453198762102489098').setName(`Üye Sayısı: ${humans}`) let bots = member.guild.members.filter(m => m.user.bot).size; bot.channels.get('453199063958159370').setName(`Bot Sayısı: ${bots}`)	const members = member.guild.memberCount;	const channel = member.guild.channels.find('name', 's_ekleyenler');	if (!channel) return;	 let Role = member.guild.roles.find(`name`, "Bot"); if(member.user.bot){	member.addRole(Role.id) }else{ let role = member.guild.roles.find(`name`, "Üye");	member.addRole(role.id) } 	let Embed = new Discord.RichEmbed()	.setFooter(`Katıldı | Üye- ${member.guild.memberCount}`)	.setColor("#cde246") 	.setAuthor(`${member.displayName} isimli üye ${member.guild.name} sunucusuna katıldı.`, member.user.displayAvatarURL)	.setTimestamp()	channel.send(Embed);	});	bot.on('guildMemberRemove', member => { bot.channels.get('453199237459738625').setName(`Toplam Üye: ${member.guild.memberCount}`) let humans = member.guild.members.filter(m => !m.user.bot).size; bot.channels.get('453199322255851521').setName(`Üye Sayısı: ${humans}`) let bots = member.guild.members.filter(m => m.user.bot).size; bot.channels.get('453199491835887616').setName(`Bot Sayısı: ${bots}`)	const channel = member.guild.channels.find(`name`, 's_atanlar');	if(!channel) return; 	let Embed = new Discord.RichEmbed()	.setColor("#e26346")	.setAuthor(`${member.displayName} isimli üye ${member.guild.name} isimli sunucudan ayrıldı`, member.user.displayAvatarURL)	.setTimestamp()	.setFooter(`Ayrıldı | Üyeler- ${member.guild.memberCount}`)	channel.send(Embed);	});
